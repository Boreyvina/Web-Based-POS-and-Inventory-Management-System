...
const { consumeStock, returnStock } = require('../utils/stock');
...
exports.checkout = asyncHandler(async (req, res) => {
  ...
  const saleId = await withTransaction(async (conn) => {
    ...
    for (const l of lines) {
      ...
      // Take the units from the batches, oldest expiry first. This updates the
      // batch rows, recalculates the product's stock and expiry date, and
      // writes one inventory log per batch touched.
      await consumeStock(conn, {
...
exports.voidSale = asyncHandler(async (req, res) => {
  ...
  await withTransaction(async (conn) => {
    ...
    for (const item of items) {
      ...
      // Find which batches this sale actually took from, so the units go back
      // with their original expiry date rather than becoming undated stock.
      const [taken] = await conn.query(
        `SELECT batch_id, SUM(-quantity_change) AS qty
         FROM inventory_logs
         WHERE reference_id = ? AND product_id = ? AND change_type = 'sale' AND batch_id IS NOT NULL
         GROUP BY batch_id`,
        [sale.id, item.product_id]
      );

      const note = `Void ${sale.invoice_no}${reason ? `: ${reason}` : ''}`;

      if (taken.length === 0) {
        await returnStock(conn, {
          productId: item.product_id, quantity: item.quantity,
          userId: req.user.id, referenceId: sale.id, note,
        });
      } else {
        for (const t of taken) {
          await returnStock(conn, {
            productId: item.product_id, quantity: Number(t.qty), batchId: t.batch_id,
            userId: req.user.id, referenceId: sale.id, note,
          });
        }
      }
