...
/**
 * GET /api/reports/by-cashier?from=&to=
 * What each member of staff took over the period. Voided sales are counted
 * separately rather than hidden, because a cashier voiding many sales is
 * something an owner should be able to see.
 */
exports.byCashier = asyncHandler(async (req, res) => {
  const { from, to } = range(req.query);
  ...
  const rows = await query(
    `SELECT u.id AS user_id, u.full_name, u.username, r.name AS role,
            ...
            COALESCE(SUM(CASE WHEN s.status = 'completed' THEN s.discount_amount END), 0) AS discounts,
            COUNT(CASE WHEN s.status = 'voided' THEN 1 END)                       AS voided,
            MAX(s.created_at)                                                     AS last_sale
     FROM users u
     JOIN roles r ON r.id = u.role_id
     LEFT JOIN sales s ON s.user_id = u.id AND s.created_at BETWEEN ? AND ?
     WHERE r.name <> 'customer'
     GROUP BY u.id, u.full_name, u.username, r.name
     HAVING transactions > 0 OR voided > 0
     ORDER BY revenue DESC`,
    [from, to]
  );

  const total = rows.reduce((n, r) => n + Number(r.revenue), 0);

  res.json({
    success: true,
    data: {
      labels: rows.map((r) => r.full_name),
      datasets: [{ label: 'Takings', data: rows.map((r) => Number(r.revenue)) }],
      rows: rows.map((r) => ({
        ...r,
        // Share of the period's takings, so one busy day does not look like
        // one busy person.
        share: total > 0 ? Math.round((Number(r.revenue) / total) * 1000) / 10 : 0,
        average_sale: r.transactions > 0 ? Number(r.revenue) / r.transactions : 0,
      })),
      total,
    },
  });
