...
## Sanity check before you run it

```bash
npm run check
```

Confirms every route points at a controller function and validator that
actually exists. Express only reports this when the file loads, with a message
that does not name the missing function, so this saves a confusing crash.
