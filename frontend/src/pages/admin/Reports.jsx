...
export default function Reports() {
  ...
  const loadSales = useCallback(() => {
    ...
    Promise.all([
      ...
      reportApi.byCashier(params),
    ])
      .then(([revenue, top, byCategory, payments, byCashier]) =>
        setSales({ loading: false, error: '', data: { revenue, top, byCategory, payments, byCashier } })
      )
          ...
          <>
            ...
            {sales.data && (
              <>
                ...
                <Panel title="Takings by staff" hint="Completed sales in this period">
                  {sales.data.byCashier.rows.length === 0 ? (
                    <p className="py-8 text-center text-sm text-slate-500">
                      Nobody rang up a sale in this period.
                    </p>
                  ) : (
                      ...
                      <table className="w-full text-sm">
                        <thead className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-500">
                          <tr>
                            ...
                            <th className="py-2 text-right font-medium">Average sale</th>
                            <th className="py-2 text-right font-medium">Discounts</th>
                            <th className="py-2 text-right font-medium">Voided</th>
                            <th className="py-2 text-right font-medium">Takings</th>
                            <th className="py-2 font-medium">Share</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {sales.data.byCashier.rows.map((c) => (
                            <tr key={c.user_id}>
                              <td className="py-2">
                                <p className="font-medium">{c.full_name}</p>
                                <p className="tnum text-xs capitalize text-slate-500">{c.username} · {c.role}</p>
                              </td>
                              <td className="tnum py-2 text-right text-slate-600">{number(c.transactions)}</td>
                              <td className="tnum py-2 text-right text-slate-600">{money(c.average_sale)}</td>
                              <td className="tnum py-2 text-right text-slate-600">{money(c.discounts)}</td>
                              <td className="tnum py-2 text-right">
                                {c.voided > 0
                                  ? <span className="font-medium text-warn-fg">{number(c.voided)}</span>
                                  : <span className="text-slate-400">—</span>}
                              </td>
                              <td className="tnum py-2 text-right font-semibold">{money(c.revenue)}</td>
                              <td className="py-2">
                                <div className="flex items-center gap-2">
                                  <div className="h-1.5 w-16 overflow-hidden rounded-full bg-slate-100">
                                    <div className="h-full bg-brand-500" style={{ width: `${c.share}%` }} />
                                  </div>
                                  <span className="tnum text-xs text-slate-500">{c.share}%</span>
                                </div>
                              </td>
                            </tr>
                          ))}
                        ...
                        <tfoot className="border-t border-slate-200">
                          <tr>
                            ...
                            <td className="tnum py-2 text-right font-semibold">{money(sales.data.byCashier.total)}</td>
                            <td />
                          </tr>
                        </tfoot>
                      </table>
                  ...
                  <p className="mt-3 text-xs text-slate-500">
                    Voided sales are shown separately rather than removed, so a pattern of
                    cancellations is visible.
                  </p>
                </Panel>
