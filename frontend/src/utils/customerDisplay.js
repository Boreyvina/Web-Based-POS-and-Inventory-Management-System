...
/**
 * Called by the till when the cashier selects QR payment, so the customer can
 * scan from their own side of the counter instead of the cashier turning the
 * till screen round. Pass null to take the code away again.
 */
export function publishPayment(payment) {
  getChannel()?.postMessage({ type: 'payment', payload: payment, at: Date.now() });
}
