 ...
 */
export default function PaymentQr({ amount, reference, onReady }) {
  ...
  useEffect(() => {
    ...
    paymentApi
      ...
      .then((data) => {
        ...
        // Hand it up so the till can mirror it onto the customer screen.
        if (onReady) onReady({ ...data, amount });
      })
      .catch((err) => {
        if (!cancelled) {
    ...
    // onReady is intentionally left out: the parent recreates the function on
    // every render, and including it would refetch the code endlessly.
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-lg bg-slate-50 p-4">
      ...
      <p className="text-center text-xs text-slate-500">
        Also showing on the customer screen. Only press Take payment once you have seen
        the money arrive in your banking app.
      </p>
    </div>
  );
}
